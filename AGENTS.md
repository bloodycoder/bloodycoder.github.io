...
以后当作者给出一篇未整理的日记 Markdown，或要求“按博客格式整理/发布”时，优先把文章整理成下面这种格式。该模板来自 `content/posts/2023_09_20.md`。
...
- Front Matter 使用 TOML，必须用 `+++` 包裹。
- `title` 必填，来自文章主题；没有明确标题时，根据正文概括一个短标题。
- `date` 必填，格式固定为 `YYYY-MM-DD`；如果用户没给日期，使用当前日期。
- `description` 必填，写一句短摘要；没有摘要时，根据正文概括。
- `tags` 必填，使用数组格式；没有明确标签时默认 `tags = ["diary"]`。
- Front Matter 后保留空行，然后放 `{{< toc >}}`，再空一行写正文。
- 正文段落之间保留一个空行。
- 不要把正文内容改写得失去作者原意；只做格式整理、错别字/标点轻微修正、Markdown 结构化。
- 如果正文里有图片，图片文件应放在 `static/images/` 或 `static/img/`，文章里用 `/images/xxx` 或 `/img/xxx` 引用。
